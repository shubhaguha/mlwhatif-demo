...
train_file = os.path.join(str(get_project_root()), "pipelines", "datasets", "compas", "compas_train.csv")
train_data = pd.read_csv(train_file, na_values='?', index_col=0)
test_file = os.path.join(str(get_project_root()), "pipelines", "datasets", "compas", "compas_test.csv")
...
featurizer = ColumnTransformer(transformers=[
    ('impute1_and_onehot', impute1_and_onehot, ['is_recid', 'c_charge_degree']),
    ('impute2_and_bin', impute2_and_bin, ['age', 'priors_count'])
])
